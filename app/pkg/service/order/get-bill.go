...
func (s *orderService) GetBill(orderID string) (bill model.Bill, status *model.Status) {
	...
	additional, err := s.AdditionalCostRepo.GetAdditionalCostByOrderID(order.ID)
	if err != nil {
		return bill, &model.Status{
			Code:    http.StatusInternalServerError,
	...
	discounts, err := s.DiscountRepo.GetDiscountsByOrderID(order.ID)
	if err != nil {
		return bill, &model.Status{
			Code:    http.StatusInternalServerError,
	...
	orderUsers, err := s.OrderUserRepo.GetOrderUserByOrderID(order.ID)
	if err != nil {
		return bill, &model.Status{
			Code:    http.StatusInternalServerError,
	...
	for _, orderUser := range orderUsers {
		...
		orderUserItems, err := s.OrderUserItemRepo.GetOrderUserItemByOrderUserID(orderUser.ID)
		if err != nil {
			return bill, &model.Status{
				Code:    http.StatusInternalServerError,
	...
	go func() {
		...
		setBillCache := s.Redis.Set(s.Redis.Context(), tool.BillRedisKey(orderID), string(billBytes), constant.RedisExpiredDuration)
